$(document).ready(function() {
    ...
    function checkedBox (category, selectedFields) {
        if(selectedFields.findIndex((item) => item.title === category) > -1 ) {
            return true;
        } else {
            return false;
        }
    ...
    function renderAvailableFields () {
        ...
        $.each(availableFields[0].categories, function(key, category){
            ...
            catInput.click(function(e){
                if($(this).is(":checked")) {
                    addField($(this), availableFields[0].parents[0].title);
                } else {
                    deleteField($(this), availableFields[0].parents[0].title);
                }
            });
        })
    }
    ...
    function renderSelectedFields (){
        ...
        $.each(groups, function(key, group){ 
            ...
            var parentDiv = "<div class='parent-title'>" + parentCategory + "</div>";
            $(".selected-fields-list").append(parentDiv);
    ...
    function deleteField (field){
        var selectedField = field.closest("label").text();
        var index = selectedFields.findIndex((item) => item.title === selectedField);     
